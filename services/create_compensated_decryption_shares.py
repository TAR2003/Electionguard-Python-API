...
def create_compensated_decryption_service(
    ...
    def _lookup_backup(backups_map: Dict[str, Any], guardian_id: str) -> Optional[Any]:
        if not isinstance(backups_map, dict):
            return None
        if guardian_id in backups_map:
            return backups_map[guardian_id]
        for k, v in backups_map.items():
            if str(k) == str(guardian_id):
                return v
        return None

    # Correct direction for compensated decryption:
    # use the backup generated by the *missing* guardian for the *available* guardian.
    # available guardian decrypts it with its own private key to recover missing coordinate.
    backup_data = _lookup_backup(missing_guardian_data.get('backups', {}), available_guardian_id)

    # Backward compatibility fallback for older persisted payload shapes.
    if not backup_data:
        backup_data = _lookup_backup(available_guardian_data.get('backups', {}), missing_guardian_id)

    if not backup_data:
        raise ValueError(
            f"No usable backup found: expected missing guardian {missing_guardian_id} to provide backup for available guardian {available_guardian_id}"
        )
