...
def deserialize_list_of_strings_to_list_of_dicts(data):
    ...
    if isinstance(data, list):
        ...
        if isinstance(data[0], dict):
            ...
            raise ValueError(
                f"Expected list of strings or dicts, got list of {type(data[0])}")
    elif isinstance(data, str):
...
def fast_encrypt_with_master_key(plaintext: bytes) -> bytes:
    ...
    cipher = Cipher(algorithms.AES(MASTER_KEY), modes.GCM(
...
def api_setup_guardian():
    ...
    try:
        print('called setup guardian call in the microservice')
        data = request.json
        guardian_id = data['guardian_id']
        sequence_order = safe_int_conversion(data['sequence_order'])
        public_key = data['public_key']  # Guardian's public key
        ...
        print_json(data, "setup_guardian")
        print_data(data, "./io/setup_guardian_data.json")

        # Call service function to setup guardian with public key only
        ...
        # Store guardian data
        if 'guardians_setup' not in election_data:
            election_data['guardians_setup'] = []
        election_data['guardians_setup'].append(result['guardian_data'])

        response = {
            'status': 'success',
            'guardian_id': result['guardian_id'],
            'sequence_order': result['sequence_order'],
            'guardian_data': serialize_dict_to_string(result['guardian_data']),
            'election_public_key': serialize_dict_to_string(result['election_public_key']),
            'number_of_guardians': result['number_of_guardians'],
            'quorum': result['quorum']
        }

        print_json(response, "setup_guardian_response")
        print_data(response, "./io/setup_guardian_response.json")
        print('Finished setup guardian call at the microservice')
...
def api_setup_guardians():
    ...
    try:
        ...
        election_data['manifest'] = create_election_manifest(
...
def api_create_encrypted_ballot():
    ...
    try:
        ...
        number_of_guardians = safe_int_conversion(
            data.get('number_of_guardians', 1))
...
def api_create_encrypted_tally():
    ...
    try:
        ...
        joint_public_key = data['joint_public_key']  # Expecting string
        commitment_hash = data['commitment_hash']    # Expecting string
        # List of encrypted ballot strings
...
def api_create_partial_decryption():
    ...
    try:
        ...
        if data.get('guardian_data'):
            try:
                guardian_data = deserialize_string_to_dict(
                    data['guardian_data'])
            except Exception as e:
        ...
        try:
            ciphertext_tally_json = deserialize_string_to_dict(
                data['ciphertext_tally'])
        except Exception as e:
        ...
        try:
            submitted_ballots_json = deserialize_list_of_strings_to_list_of_dicts(
                data['submitted_ballots'])
        except Exception as e:
...
def api_create_compensated_decryption():
    ...
    try:
        ...
        try:
            available_guardian_data = deserialize_string_to_dict(
                data['available_guardian_data'])
        except Exception as e:
            raise ValueError(
                f"Error deserializing available_guardian_data: {e}")

        try:
            missing_guardian_data = deserialize_string_to_dict(
                data['missing_guardian_data'])
        except Exception as e:
            raise ValueError(f"Error deserializing missing_guardian_data: {e}")

        try:
            available_private_key = deserialize_string_to_dict(
                data['available_private_key'])
        except Exception as e:
            raise ValueError(f"Error deserializing available_private_key: {e}")

        try:
            available_public_key = deserialize_string_to_dict(
                data['available_public_key'])
        except Exception as e:
            raise ValueError(f"Error deserializing available_public_key: {e}")

        try:
            available_polynomial = deserialize_string_to_dict(
                data['available_polynomial'])
        except Exception as e:
...
def api_combine_decryption_shares():
    ...
    try:
        ...
        try:
            guardian_data = deserialize_list_of_strings_to_list_of_dicts(
                data['guardian_data'])
        except Exception as e:
            raise ValueError(f"Error deserializing guardian_data: {e}")
        ...
        available_guardian_public_keys = data.get(
        ...
        for i, guardian_id in enumerate(available_guardian_ids_list):
            try:
                ...
                raise ValueError(
                    f"Error reconstructing available_guardian_shares for {guardian_id}: {e}")
        ...
        compensating_guardian_ids_list = data.get(
        ...
        print(
            f"Quorum required: {quorum}, Available: {len(available_guardian_ids)}")

        # Validate we have enough guardians
        if len(available_guardian_ids) < quorum:
            raise ValueError(
                f"Insufficient guardians available. Need {quorum}, have {len(available_guardian_ids)}")
        ...
        for missing_guardian_id in missing_guardian_ids:
            if missing_guardian_id in all_compensated_shares:
                ...
                print(
                    f"Including compensated shares for missing guardian: {missing_guardian_id}")
            else:
                raise ValueError(
                    f"Missing compensated shares for guardian {missing_guardian_id}")

        # Log what we're filtering out
        excluded_guardians = set(
            all_compensated_shares.keys()) - missing_guardian_ids
        if excluded_guardians:
            print(
                f"Excluding compensated shares for available guardians: {sorted(excluded_guardians)}")
...
def encrypt_it():
    ...
    try:
        ...
        nonce = os.urandom(12)
        cipher = Cipher(algorithms.AES(combined_key),
                        modes.GCM(nonce), backend=default_backend())
        encryptor = cipher.encryptor()

        private_key_bytes = private_key.encode('utf-8')
        encrypted_data = encryptor.update(
            private_key_bytes) + encryptor.finalize()

        # Fast password encryption
        encrypted_password = fast_encrypt_with_master_key(
        ...
        credentials_json = json.dumps(
            credentials_data, separators=(',', ':')).encode('utf-8')
        ...
        return jsonify({
            ...
            # Storage 2 (includes HMAC)
            'credentials': base64.b64encode(final_credentials).decode()
        }), 200
...
def decrypt_it():
    ...
    try:
        ...
        credentials_for_verification = {
            k: v for k, v in credentials.items() if k != 'hmac_tag'}
        credentials_for_verification_json = json.dumps(
            credentials_for_verification, separators=(',', ':')).encode('utf-8')
        ...
        password = fast_decrypt_with_master_key(
        ...
        if not verify_hmac(hmac_key, credentials_for_verification_json, hmac_tag):
            logger.warning(
                f"HMAC verification failed for IP: {request.remote_addr}")
